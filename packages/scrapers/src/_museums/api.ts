...
import type { ProxyConfig as PackagesProxyConfig } from "../proxy";
import { proxyFetch as packagesProxyFetch } from "../proxy";
import type { EventApiType, ExhibitionApiType, ProxyConfig } from "./config";
...
function truncateHtml(text: string, maxLen = 500): string | null {
  return truncate(text, maxLen);
}

// Adapt the apps' proxyFetch(url, proxy) shape to the packages-level
// proxyFetch(targetUrl, proxy | null, init?) signature without touching the
// 50+ call sites below.
function proxyFetch(url: string, proxy: ProxyConfig): Promise<Response> {
  const adapted: PackagesProxyConfig = { url: proxy.url, token: proxy.token };
  return packagesProxyFetch(url, adapted);
}
