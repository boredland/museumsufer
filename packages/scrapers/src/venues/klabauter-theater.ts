 ...
 * `<p>` paragraphs carry the production title, an optional "INFO" / "zum
 * Stück" link, and date lines `<strong>[Wochentag, ]DD.MM.YYYY </strong>|
 * HH.MM Uhr | Label` (the weekday prefix appeared in the 2026/27 season). A
 * single date line may list several showtimes (`18.00, 19.00 & 20.00 Uhr`);
 * each becomes its own performance. Reachable only over the apex domain —
 * the `www.` host's TLS cert is broken, which is why direct fetches to it
 * fail.
 */
...
function addShows(chunk: string, events: CanonicalScrapedEvent[], seen: Set<string>, today: string): void {
  ...
  // A performance row is `[Wochentag, ]DD.MM.YYYY | HH.MM[, HH.MM…] Uhr | Label`.
  const dateLine = /^(?:[A-Za-zäöüÄÖÜ]+,\s*)?(\d{2})\.(\d{2})\.(\d{4})\s*\|\s*([^|]*?)\s*(?:\|\s*(.*))?$/;
  const firstDateIdx = lines.findIndex((l) => dateLine.test(l));
  if (firstDateIdx < 0) return;

  // The detail link's label ("INFO", "zum Stück") is a line of its own or
  // trails the audience note ("ab 2 Jahre | INFO"); neither is title text.
  const titleLines = lines
    .slice(0, firstDateIdx)
    .map((l) => l.replace(/\s*\|\s*(?:INFO|zum (?:Stück|Festival))$/i, ""))
    .filter((l) => l && !/^(?:zum (?:Stück|Festival)|INFO)$/i.test(l) && !/^Ort:/i.test(l));
