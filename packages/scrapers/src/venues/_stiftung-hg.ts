...
/** Lecture-style events all use 'Medizin im Fokus' as a prefix; other
 *  cards (Trauergruppe, Familientage, Schwangerschaftskurse) get the
 *  generic talk:vortrag label since they're still patient-public events. */
...
export async function scrapeStiftungHg(): Promise<VenueScrapeResult[]> {
  ...
  for (const m of html.matchAll(ARTICLE_RE)) {
    ...
    const labels: ScrapedLabel[] = [
      LECTURE_PREFIX_RE.test(title)
        ? { label: "talk:vortrag", confidence: 0.95, classifier: "scraper-hardcoded" }
