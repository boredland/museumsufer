...
const API_URL = "https://backend.premiumkino.de/v1/de/savoy/program";
const SITE_BASE = "https://savoy.premiumkino.de";
...
export async function scrapeSavoyFilmtheater(): Promise<VenueScrapeResult> {
  const today = todayIso();
  const res = await fetch(API_URL, { headers: { Accept: "application/json" } });
  if (!res.ok) throw new Error(`savoy filmtheater fetch failed: ${res.status}`);
  ...
  return { source_slug: "savoy-filmtheater", display_name: "Savoy Filmtheater", events };
}
