...
const API_URL = "https://backend.premiumkino.de/v1/de/hamburg/program";
const SITE_BASE = "https://hafencity.premiumkino.de";
...
export async function scrapeAstorHafencity(): Promise<VenueScrapeResult> {
  const today = todayIso();
  const res = await fetch(API_URL, { headers: { Accept: "application/json" } });
  if (!res.ok) throw new Error(`astor hafencity fetch failed: ${res.status}`);
  ...
  return { source_slug: "astor-hafencity", display_name: "ASTOR Film Lounge HafenCity", events };
}
