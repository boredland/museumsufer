import { classifyMusic, type MusicGenre } from "@museumsufer/classify";
import {
  berlinNow,
  dateOffset,
  decodeEntities,
...
const HR_UA = "museumsufer event-hub crawler / contact: jonas@bgdlabs.com";
...
export interface HrVenueConfig {
  ...
  /** Stable id prefix for source_event_id (keeps cross-venue uniqueness when both share a date). */
  slugPrefix: string;
}

export async function scrapeHrVenue(cfg: HrVenueConfig): Promise<VenueScrapeResult> {
  ...
  for (const card of ffm) {
    ...
    if (detailFetches < HR_MAX_DETAIL_FETCHES) {
      ...
      try {
        ...
        console.warn(`${cfg.sourceSlug} detail fetch failed for ${card.detailUrl}:`, err);
      }
      detailFetches++;
    }
...
async function fetchMonthHtml(cfg: HrVenueConfig, month: string): Promise<string | null> {
  ...
  if (!res.ok) throw new Error(`${cfg.sourceSlug} fetch failed: ${url} → ${res.status}`);
  return res.text();
}
