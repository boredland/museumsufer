 ...
 * Only events whose abstract carries "JAZZ IM PALMENGARTEN" are kept — the
 * rest belong to the garden's botanical programming.
 */
export async function scrapeJazzPalmengarten(): Promise<VenueScrapeResult> {
  ...
  for (let i = 0; i < windows.length; i++) {
    ...
    for (const item of items) {
      ...
      const genre = classifyMusic(title, subtitle, detail.description, "jazz");
