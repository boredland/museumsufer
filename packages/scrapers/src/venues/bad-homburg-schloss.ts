...
/**
 * Jimdo-hosted static page. WordPress REST endpoint is locked and the ztix
 * ticketing API is empty between seasons (only carries individual concert
 * show-IDs once pre-sale opens, around mid-June for the following season).
 *
 * Season schedule is plaintext on the season landing page (/konzerte-25-26/):
 *
 *   DD.MM.YYYY: <category / number> [optional "subtitle"] <performers…>
 *
 * The page transitions to previewing the next season as the current one
 * winds down. We probe the known season slugs and parse whichever returns.
 * Venue room is hardcoded to "Schlosskirche" — every concert in the series
 * uses the castle chapel. Times, prices, ticket URLs and images are not
 * published in the preview; they appear on detail pages post-pre-sale.
 */
...
export async function scrapeBadHomburgSchloss(): Promise<VenueScrapeResult> {
...
function parseSeasonListing(html: string, detailUrl: string): CanonicalScrapedEvent[] {
  ...
  for (let i = 0; i < matches.length; i++) {
    ...
    const genre = classifyMusic(title, subtitle, null, "classical");
