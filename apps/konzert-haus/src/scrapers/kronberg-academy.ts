...
export async function scrapeKronbergAcademy(): Promise<ScrapeResult> {
  ...
  for (let page = 1; page <= MAX_PAGES; page++) {
    ...
    for (const block of blocks) {
      ...
      const dedup = `${parsed.slug}|${parsed.date}|${parsed.time ?? ""}|${parsed.venue_room ?? ""}`;
...
function parseEvent(block: string): ScrapedEvent | null {
  ...
  return {
    ...
    time: startTime,
    end_time: endTime,
    genre: classify(title, subtitle, description, "classical"),
    image_url: null,
