  ...
  for (let page = 1; page <= MAX_PAGES; page++) {
    ...
    for (const tile of tiles) {
      ...
      events.push({
        ...
        time: parsed.time,
        end_time: null,
        genre: classify(parsed.title, parsed.category, description, "classical"),
    ...
    // The program page renders every month on a single response. If a
    // future season ever introduces real pagination we still walk it; for
    // now a page that adds zero new events ends the loop.
...
function extractPerformers(tile: string): string | null {
  // The performer paragraph is the first <p> after the <h3> title — the
  // surrounding column also contains category/date/venue paragraphs, so
  // position relative to the title is what disambiguates it.
...
function extractPrices(tile: string): { priceMin: number | null; priceMax: number | null } {
  // Two sources: the expanded `Kartenpreise` block (`NN.NN€`, one per
  // category) and the teaser's `Karten ab NN.NN €` line. The latter is
  // sometimes the only signal when the expanded block is sold out.
