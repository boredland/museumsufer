...
const EVENTS_QUERY = `
query KonzertHausHfmdkEvents($from: String!, $offset: Int!, $limit: Int!) {
  entityQuery(
    entityType: NODE
    filter: {
      conditions: [
...
export async function scrapeHfmdk(): Promise<ScrapeResult> {
  ...
  for (let page = 0; page < MAX_PAGES; page++) {
    ...
    for (const raw of items) {
      ...
      if (!event) continue;
      const dedup = `${event.slug}|${event.date}|${event.time ?? ""}`;
...
async function fetchPage(from: string, offset: number, limit: number): Promise<HfmdkEvent[]> {
  const res = await fetch(GRAPHQL_ENDPOINT, {
    ...
    body: JSON.stringify({
      operationName: "KonzertHausHfmdkEvents",
...
function transform(raw: HfmdkEvent, date: string, datum: string): ScrapedEvent | null {
  ...
  return {
    ...
    genre: classify(title, finalSubtitle, description, "classical"),
    image_url: imageUrl,
