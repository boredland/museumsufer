 ...
 * the `future-event` class filter. The WP REST API has post bodies (the
 * concert programme + performers) but no event-date custom field, so the
 * home-page card is the canonical date source.
 */

const BASE = "https://naxoshallenkonzerte.de";
const UA = "konzert.haus crawler / contact: jonas@bgdlabs.com";

// "Sa13.Jun26" — anchored to the German weekday-abbr prefix so we don't pick
// up random "13.Jun" prose elsewhere on the page.
const COMPACT_DATE_RE = /(?:Mo|Di|Mi|Do|Fr|Sa|So)\s*(\d{1,2})\.\s*([A-Za-zäöü]{2,4})\s*(\d{2})/;
// "20H00" — German abbreviation for "20 Uhr 00 Minuten"
const COMPACT_TIME_RE = /(\d{1,2})H(\d{2})/;
// Whole future-event anchor block.
...
export async function scrapeNaxos(): Promise<ScrapeResult> {
  ...
  for (const m of html.matchAll(FUTURE_EVENT_RE)) {
    ...
    // Walk all <h3>s in the card. First one matching the compact-date pattern
    // is the date, the next is the time, the rest are genre tags.
    ...
    events.push({
      ...
      time,
      end_time: null,
      genre: classify(title, null, description, "experimental"),
  ...
  return { venue_slug: "naxos-hallenkonzerte", events };
}
