  ...
  for (const group of dedupedBySlug.values()) {
    for (const r of group) {
      ...
      events.push({
        ...
        time: r.time,
        end_time: null,
        genre: classify(r.title, r.subtitle, description, r.fallbackGenre),
...
/**
 * `/de/konzerte/` and `/de/liederabende/` ship `repertoire-element-mini` cards.
 * Each card carries the full date in its `<span class="meta">` (e.g.
 * "17. Mai 2026, 11.00 Uhr, Alte Oper Frankfurt"), so we don't need the
 * `dates_available` JS array on these list pages.
 *
 * Card shape:
 *   <a href="/de/spielplan/<slug>/?id_datum=<id>" class="season-hover-text">
 *     <span class="meta">DD. Monat YYYY, HH.MM Uhr, &lt;Saal&gt;</span>
 *     <h3>&lt;Title&gt;</h3>
 *     <div class="info"><span><em>&lt;Subtitle&gt;</em></span></div>
 *   </a>
 *   <div class="element-labels"><a href="&lt;external ticket url&gt;">…</a></div>
 */
function parseListing(html: string, fallbackGenre: Genre): RawEvent[] {
...
function parseElement(block: string, fallbackGenre: Genre): RawEvent | null {
...
function extractSubtitle(block: string): string | null {
  ...
  // Prefer the <em>…</em> fragment when present; some `info` blocks carry
  // sales notices ("Vorverkauf ab …") rather than a production subtitle.
