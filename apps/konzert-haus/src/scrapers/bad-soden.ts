 ...
 *
 *   <p><span class="purple">21. Juni 2026 - "Nachtliebessehnsucht"</span></p>
 *   <p>  Chor- und Streichermusik …</p>
 *   <p>  Bürgerhaus Neuenhain, …</p>
 *   <p>  Beginn 19:00 Uhr</p>
 *   <ul><li>Antonio Vivaldi …</li>…</ul>
 *   <p><a href="https://musikstiftung.de/tickets/…">Online-Ticketverkauf</a></p>
 *
...
export async function scrapeBadSoden(): Promise<ScrapeResult> {
  ...
  return { venue_slug: "bad-soden", events: parseGdmProgram(await res.text()) };
}

export function parseGdmProgram(html: string): ScrapedEvent[] {
  ...
  for (let i = 0; i < headers.length; i++) {
    ...
    events.push({
      ...
      end_time: null,
      genre: classify(title, subtitle, description, "chamber"),
      image_url: null,
...
/**
 * Limit parsing to the article body so navigation lists ("Drucken",
 * "Impressum") and shared footer markup never leak into the trailing
 * event's metadata. The page wraps every article in a balanced `<div
 * class="copy">` … `</div>` pair; everything outside is chrome.
 */
