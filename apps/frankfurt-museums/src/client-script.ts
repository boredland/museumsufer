...
const WEBMCP_TOOLS: WebMcpToolDef[] = [
  {
    name: "get_events",
    description:
      "Get museum events for a specific date in Frankfurt. Returns event titles, times, venues, prices, and links.",
    inputSchema: {
      type: "object",
      properties: {
  ...
  },
  {
    ...
    description:
      "Get current museum exhibitions in Frankfurt. Returns exhibition titles, date ranges, venues, and links.",
    inputSchema: {
      type: "object",
      properties: {
        date: { type: "string", description: "ISO date (YYYY-MM-DD). Defaults to today." },
  ...
  },
  {
    ...
    executeBody: `return fetch('/api/museums').then(function(r) { return r.json(); });`,
  },
  {
