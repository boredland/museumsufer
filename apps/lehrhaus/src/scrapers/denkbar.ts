...
// Music performances belong in konzert.haus; drop them here.
const MUSIC_DROP_RE = /\b(jazz|blaue\s+stunde|lieder\s+und\s+arien|acapella|a\s+capella|tag\s+der\s+musik)\b/i;
...
export async function scrapeDenkbar(): Promise<ScrapedEvent[]> {
  ...
  for (const post of posts) {
    ...
    if (MUSIC_DROP_RE.test(title) || classifyEvent(title) === "Konzert") continue;
