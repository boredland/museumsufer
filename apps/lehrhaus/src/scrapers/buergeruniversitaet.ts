...
const DISKUSSION_CATS = new Set([121, 218]); // Diskussionen, Podiumsgespräch
...
export async function scrapeBuergeruniversitaet(): Promise<ScrapedEvent[]> {
  ...
  while (true) {
    ...
    for (const e of data.events) {
      ...
      events.push({
        ...
        description,
        detail_url: e.url,
        category: isDiskussion ? "Diskussion" : "Vortrag",
