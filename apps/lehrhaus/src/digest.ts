...
import { CATEGORIES, type Category, type Env } from "./types";
...
function parseFilters(json: string | null): LhFilters | null {
  if (!json) return null;
  try {
    const v = JSON.parse(json);
    if (!v || typeof v !== "object") return null;
    if (!Array.isArray((v as { formats?: unknown }).formats)) return null;
    const formats = ((v as { formats: unknown[] }).formats as unknown[]).filter(
      (c): c is Category => typeof c === "string" && (CATEGORIES as readonly string[]).includes(c),
    );
...
function eventsMatchingFilters(events: EventLike[], filters: LhFilters | null): EventLike[] {
  if (!filters?.formats || filters.formats.length === 0) return events;
  const allowed = new Set(filters.formats);
  return events.filter((e) => allowed.has(e.category));
}

function buildMorningPayload(filters: LhFilters | null): NotificationPayload | null {
  const date = todayIso();
  ...
  return {
    title: `Heute: ${events.length} Vortr${events.length === 1 ? "ag" : "äge"}`,
    body,
...
function buildAfternoonPayload(filters: LhFilters | null): NotificationPayload | null {
  const date = todayIso();
  ...
  return {
    title: `Heute Abend: ${evening.length} Vortr${evening.length === 1 ? "ag" : "äge"}`,
    body,
    url: `${APP_URL}/tag/${date}`,
    tag: `afternoon-${date}`,
  };
...
function buildWeeklyPayload(filters: LhFilters | null): NotificationPayload | null {
  const from = todayIso();
  ...
  return {
    title: `Diese Woche: ${events.length} Vorträge & Diskussionen`,
    body: peak ? `Höhepunkt ${peak[0]}: ${peak[1]} Termine` : "Wochenüberblick verfügbar",
    url: `${APP_URL}/tag/${from}`,
    tag: `weekly-${from}`,
  };
}

function buildPayload(schedule: Schedule, filters: LhFilters | null): NotificationPayload | null {
