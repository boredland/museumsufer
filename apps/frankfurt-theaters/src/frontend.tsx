...
export function renderAskAi(): string {
  ...
  const buttons = LLM_SERVICES.map(
    (s) =>
      `<a class="askai__svc" href="${s.buildUrl(prompt)}" target="_blank" rel="noopener" aria-label="${s.name}" title="${s.name}" style="color:${s.color}"><svg viewBox="0 0 24 24" width="13" height="13" aria-hidden="true" fill="currentColor"><path d="${s.svgPath}"/></svg></a>`,
  ).join("");
