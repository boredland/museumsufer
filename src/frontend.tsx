    ...
    <nav class="flex justify-center gap-1 mt-3" aria-label="Language">
      {SUPPORTED_LOCALES.map((l) => (
        <a
          href={`?lang=${l}`}
          data-lang={l}
          onclick="event.preventDefault();var u=new URL(location.href);u.searchParams.set('lang',this.dataset.lang);if(typeof currentDate!=='undefined')u.searchParams.set('date',currentDate);if(typeof sortByDistance!=='undefined'&&sortByDistance)u.searchParams.set('sort','near');else u.searchParams.delete('sort');location.href=u.toString()"
